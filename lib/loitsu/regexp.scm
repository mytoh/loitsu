...
(library (loitsu regexp)
    (export
      regexp            rx
      char-set->sre         valid-sre?
      regexp?           regexp-match
      regexp-match?         regexp-search
      regexp-fold       regexp-extract
      regexp-split          regexp-partition
      regexp-replace    regexp-replace-all
      rx-match?             rx-match-count
      rx-match-submatch rx-match-submatch-start
      rx-match-submatch-end rx-match->list)
  ...
  ;; for non chibi-scheme
  ;; from init-7.scm
  (define (string-index->offset str i) i)
  ;; from chibi/regexp.sld
