...
(library (loitsu clos bootstrap generic-functions)
  ...
  (import (only (rnrs) define quote list lambda car cdr begin let cond eq? else error)
          (only (loitsu clos bootstrap standard-classes) bootstrap-make <method> <class> <entity-class> <object>
                <generic>)
          (only (loitsu clos private allocation) set-instance-printer!)
          (only (loitsu clos introspection) class-of)
          (only (loitsu clos std-protocols make) class-make)
          (only (loitsu clos std-protocols allocate-instance) class-allocate-instance entity-class-allocate-instance)
          (only (loitsu clos std-protocols initialize) class-initialize generic-initialize method-initialize)
          (only (loitsu clos std-protocols class-initialization) class-compute-precedence-list class-compute-slots class-compute-getter-and-setter)
          (only (loitsu clos std-protocols add-method) generic-add-method)
          (only (loitsu clos std-protocols generic-invocation) generic-compute-methods generic-compute-apply-generic generic-compute-method-more-specific?
                generic-compute-apply-methods register-generic-invocation-generics!)
          (only (loitsu clos std-protocols print-object) object-print-object))
